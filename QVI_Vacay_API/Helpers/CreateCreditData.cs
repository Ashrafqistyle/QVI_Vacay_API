namespace QVI_Vacay_API.Helpers
{
    ...
    public class CreateCreditData
    {
        ...
        public bool CheckAccountId(string accounts_id)
        {
            ...
            try
            {
                var strQuery = "SELECT COUNT(accounts_id) As Count " + "FROM pt_accounts " + "WHERE accounts_id = @accounts_id ";
        ...
        public bool CreateCreditMovement(CreateCredit_Request obj)
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(strconMysql))
                {
                    ...
                    {
                        command.CommandText = "INSERT INTO credit_movement (type, creation_date, user_id, ir_id, booking_id, " + " status, credit_value, description, internal_description) " + "VALUES(@type, @creation_date, @user_id, @ir_id, @booking_id, " + " @status, @credit_value, @description, @internal_description) ";
        ...
        public string GetNewCreditMovement(CreateCredit_Request obj)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(strconMysql))
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    command.CommandText = "SELECT id " + "FROM credit_movement " + "WHERE type = @type " + "AND user_id = @user_id " + "AND ir_id = @ir_id " + "AND booking_id = @booking_id " + "AND credit_value = @credit_value " + "ORDER by creation_date Desc " + "LIMIT 1 ";
                    command.Parameters.AddWithValue("@type", "Cash In");
                    command.Parameters.AddWithValue("@user_id", obj.account_id);
                    command.Parameters.AddWithValue("@ir_id", obj.ir_id);
        ...
        public bool CreateCreditRepository(CreateCredit_Request obj, string credit_movement_id)
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(strconMysql))
                {
                    ...
                    using (MySqlCommand command = new MySqlCommand("", connection))
                    {
                        command.CommandText = "INSERT INTO credit_repository (user_id, credit_movement_id, credit_original_value, credit_balance_value, expiration_date, " + " credit_usage_info, creation_date, modified_date, order_no, item_no) " + "VALUES(@user_id, @credit_movement_id, @credit_original_value, @credit_balance_value, @expiration_date, " + " @credit_usage_info, @creation_date, @modified_date, @order_no, @item_no) ";
        ...
        public double GetAccountBalance(string accounts_id)
        {
            ...
            try
            {
                using (MySqlConnection connection = new MySqlConnection(strconMysql))
                {
                    ...
                    using (MySqlCommand command = new MySqlCommand("", connection))
                    {
                        command.CommandText = "SELECT SUM(credit_balance_value) As AccountBalance " + "FROM credit_repository " + "WHERE user_id = @user_id " + "AND expiration_date > @CurrentDateTime ";
        ...
        public bool UpdateAccountBalance(string accounts_id, double balance)
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(strconMysql))
                {
                    ...
                    using (MySqlCommand command = new MySqlCommand("", connection))
                    {
                        command.CommandText = "UPDATE pt_accounts " + " SET balance = @balance " + " WHERE accounts_id  = @accounts_id ";
