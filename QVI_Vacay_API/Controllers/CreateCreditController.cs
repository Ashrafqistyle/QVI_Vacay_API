...
namespace QVI_Vacay_API.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class CreateCreditController : ControllerBase
    {
        ...
        private readonly ILogger<CreateCreditController> _logger;

        public CreateCreditController(ILogger<CreateCreditController> logger)
        {
            _logger = logger;
        }
        ...
        [HttpPost]
        public CreateCredit_Response PostValue(CreateCredit_Request obj)
        {
            var re = Request;
            var headers = re.Headers;
            string currentToken = string.Empty;
            CreateCredit_Response response = new CreateCredit_Response();

            if (obj != null)
            {
                CreateCreditData data = new CreateCreditData();
                ...
                bool createCreditStatus = false;
                bool createCreditRepoStatus = false;
                ...
                if (checkTokenStatus == true)
                {
                    ...
                    if (updateTokenStatus == true)
                    {
                        ...
                        {
                            createCreditStatus = data.CreateCreditMovement(obj);

                            if (createCreditStatus == true)
                            {
                                creditMovementId = data.GetNewCreditMovement(obj);

                                if (creditMovementId != "")
                                {
                                    createCreditRepoStatus = data.CreateCreditRepository(obj, creditMovementId);

                                    if (createCreditRepoStatus == true)
                                    {
